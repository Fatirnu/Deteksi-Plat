...
UPLOAD_FOLDER = 'uploads/Kendaraan'
ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg', 'gif'}
PLAT = 'uploads/Plat'
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['PLAT'] = PLAT
...
def process_image(file_path):
    ...
    for contour_plate in contours_plate:
        ...
        if h_char >= 40 and h_char <= 100 and w_char >=10:
    ...
    if index_chars_candidate == []:
        ...
        terdeteksi = cv.putText(img_show_plate, plate_number,(x_plate, y_plate + h_plate + 50), cv.FONT_ITALIC, 2.0, (0,255,0), 3)
        
        # cv.imshow(plate_number, img_show_plate)
    cv.waitKey(0)

    return img_plate_bw_rgb, img_show_plate_bw, img_show_plate
...
def index():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            # # Proses gambar
            processed_image_bw, processed_image_show, hasil = process_image(file_path)
            # Save the images to display in the template
            processed_image_bw_path = os.path.join(app.config['UPLOAD_FOLDER'], 'processed_image_bw.jpg')
            cv.imwrite(processed_image_bw_path, processed_image_bw)

            processed_image_show_path = os.path.join(app.config['UPLOAD_FOLDER'], 'processed_image_show.jpg')
            cv.imwrite(processed_image_show_path, processed_image_show)

            terdeteksi_image_show_path = os.path.join(app.config['UPLOAD_FOLDER'], 'terdeteksi_image_show.jpg')
            cv.imwrite(terdeteksi_image_show_path, hasil)

        return render_template('index.html', processed_image_bw_path=processed_image_bw_path,
                               processed_image_show_path=processed_image_show_path,
                               terdeteksi_image_show_path=terdeteksi_image_show_path)
                            #    terdeteksi_image_show_path=terdeteksi_image_show_path)
            ...
            # result_image_path = os.path.join(app.config['PLAT'], 'result_image.jpg')
            # cv.imwrite(result_image_path, img_show_plate)
...
# @app.route('/uploads/result_image/<filename>')
# def uploaded_Plat(filename):
#     return send_from_directory(app.config['PLAT'], filename)
